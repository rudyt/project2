...
#     ],
     'help':[
     {'user':'admin','msg':'submit your support request here','time':'0'}
     ],
     'AllUsers':[
...
def login():
    if request.method == 'POST':
        ...
        channel = 'AllUsers'
        if username not in users and username != None:
...
def registeruser(data):
    ...
    emit('announce disconnected',data,broadcast=True)
...
def submitmsg(data):
    ...
    data['time'] = curTime
    emit('submit msg done',data,broadcast=True)
